...
/** System / geometry / editor fields that should never appear as indicators. */
...
/**
 * Scale / relationship companion fields (e.g. Crime_S100, Crime_S_100).
 * These are linked internally and must not be treated as independent indicators.
 */
...
/** True when the layer field is a usable numeric indicator attribute. */
function isCandidateNumericField(f: { name?: string; alias?: string; type?: string }): boolean {
  const type = String(f.type ?? "").trim();
  if (!type) {
    // Some layers expose fields before type is populated — include them.
    return true;
  }
  ...
  // Broader match for ArcGIS REST / JS API type string variants.
  return /integer|double|single|float|long|short|number|count|oid/i.test(type);
}

/**
 * Numeric attribute fields present on web-map feature layers but not registered
 * in any indicator group (and not admin name/code fields). Skips *_S100 / *S_100
 * relationship/scale companions.
 */
function collectUndefinedFields(catalog: IndicatorGroup[]): IndicatorField[] {
  ...
  for (const g of catalog) {
    for (const f of g.fields) {
      ...
      // Base name without _S100 also counts as assigned for the companion.
  ...
  for (const layer of map.featureLayers()) {
    ...
    for (const f of fields) {
      ...
      // Skip if base name is already assigned (e.g. Crime registered → hide Crime_S100).
        ...
        <div className="data-update-body">
          ...
          {mainTab === "add" ? (
            <>
              ...
              <div className="data-update-add-sticky">
                <section>
                  <div className="section-head">
            ...
            <div className="data-update-scroll">
              ...
              {modifyTab === "modify_field" ? (
                <section className="data-update-card data-update-card--flat">
                  <p className="section-hint" style={{ marginTop: 0 }}>
                    Move or remove fields between indicator groups. Undefined Fields lists numeric
                    attributes on the map that are not yet assigned to any group — pick a group to
                    register them. Changes apply to the dashboard catalog (Smart Symbology and
                  ...
                  <details className="data-update-group-block">
                    ...
                    {undefinedFields.length ? (
                          ...
                          <li key={f.id} className="data-update-field-row" role="listitem">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (to) assignUndefinedField(f, to);
                              }}
                            >
                      ...
                      <p className="empty-note">
                        {mapReady
                          ? "No unassigned numeric fields on the map layers."
                          : "Wait for the map to load to detect unassigned fields."}
                      </p>
                    )}
                  </details>
                </section>
