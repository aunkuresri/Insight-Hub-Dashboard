...
function buildBivariate(args: {
  ...
  const [breakX1, breakX2] = tertileBreaks(valuesX);
  const [breakY1, breakY2] = tertileBreaks(valuesY);
  const breaksX = [breakX1, breakX2];
  const breaksY = [breakY1, breakY2];
  ...
  const levelRange = (b1: number, b2: number, cls: number): string => {
    if (cls === 1) return `≤ ${formatBreak(b1)}`;
    if (cls === 2) {
      if (b1 === b2) return `= ${formatBreak(b1)}`;
      return `${formatBreak(b1)} – ${formatBreak(b2)}`;
    }
    return `> ${formatBreak(b2)}`;
  };
  const levelsX: Record<number, string> = {
    1: levelRange(breakX1, breakX2, 1),
    2: levelRange(breakX1, breakX2, 2),
    3: levelRange(breakX1, breakX2, 3),
  };
  const levelsY: Record<number, string> = {
    1: levelRange(breakY1, breakY2, 1),
    2: levelRange(breakY1, breakY2, 2),
    3: levelRange(breakY1, breakY2, 3),
  };
