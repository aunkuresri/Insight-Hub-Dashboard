...
function buildChoropleth(args: {
  ...
  if (values.length < 2 || values[0] === values[values.length - 1]) {
    ...
    return {
      ...
      legend: {
        ...
        colors: { fill: rgb },
        classes: [{ label, color: rgb }],
      },
    };
  }

  // Prefer up to 5 classes, but never more than unique value count allows.
  const uniqueCount = new Set(values).size;
  const classCount = Math.min(5, ramp.length, Math.max(2, uniqueCount));
  const rawBreaks = quantileBreaks(values, classCount);

  // Collapse duplicate edges (common when many features share 0 or the same count).
  const edgeSet = new Set<number>();
  edgeSet.add(values[0]!);
  for (const b of rawBreaks) edgeSet.add(b);
  edgeSet.add(values[values.length - 1]!);
  const edges = [...edgeSet].sort((a, b) => a - b);

  // Need at least 2 edges to form one class; if collapsed, fall back to min/max pair.
  if (edges.length < 2) {
    const rgb = SINGLE_SYMBOL_COLORS[requestedScheme] ?? SINGLE_SYMBOL_COLORS.Auto;
    return {
      renderer: {
      ...
      legend: {
        ...
        fields: [{ id: field, name: field, label }],
        colors: { fill: rgb },
        classes: [{ label: formatBreak(values[0]!), color: rgb }],
      },
    };
  }

  const actualClasses = edges.length - 1;
  ...
  for (let i = 0; i < actualClasses; i += 1) {
    ...
    // Spread ramp colors across the actual number of classes (not forced 5).
    const colorIndex =
      actualClasses <= 1
        ? 0
